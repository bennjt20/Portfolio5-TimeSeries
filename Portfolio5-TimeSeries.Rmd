...
After creating my final visualization, then attempting to knit the file, R gave me an error message on line 12, so I removed this command from the knitted version:
...
2) To isolate the specific years 2015-2019, we can use the following code. 
```{r Scatterplot1}
DateRange [TimeSeries$Time >= "2015" & TimeSeries$Time <= "2019", ]
```

This generates a tibble, which we will need to convert into a reusable dataframe. 
